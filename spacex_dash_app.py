...
def get_scatter_chart(entered_site, payload_mass):
    filtered_df = spacex_df
    if entered_site == 'ALL':
        fig = px.scatter(filtered_df[filtered_df['Payload Mass (kg)'].isin(range(payload_mass[0],payload_mass[1]))], x="Payload Mass (kg)", y="class", color="Booster Version Category") 
        return fig
    else:
        # return the outcomes piechart for a selected site
        fig = px.scatter(filtered_df[(filtered_df['Launch Site']==entered_site) & (filtered_df['Payload Mass (kg)'].isin(range(payload_mass[0],payload_mass[1])))], x="Payload Mass (kg)", y="class", color="Booster Version Category") 
        return fig
